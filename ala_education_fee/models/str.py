...
class StudentFees(models.Model):
    ...
    @api.depends('fee_line_ids', 'fee_line_ids.amount', 'fee_line_ids.payment_mode', 'fee_line_ids.invoice_id',
                 'fee_line_ids.amount', 'fee_line_ids.amount_to_paid')
    def compute_total(self):
        for rec in self:
            ...
            rec.amount_paid = sum(
                line.amount_to_paid for line in rec.fee_line_ids.filtered(lambda r: r.payment_status == 'paid'))
            amount_unpaid = sum(
                line.amount_to_paid for line in rec.fee_line_ids.filtered(lambda r: r.payment_status == 'unpaid'))
            rec.amount_due = sum(
                line.amount_to_paid for line in rec.fee_line_ids.filtered(lambda r: r.payment_status == 'over_due'))
            rec.amount_upcoming = sum(
                line.amount_to_paid for line in rec.fee_line_ids.filtered(lambda r: r.payment_status == 'upcoming'))
...
class StudentFeeLine(models.Model):
    ...
    register_number = fields.Char(related='student_fee_id.register_number', string='Register Number', readonly=True,
                                  store=True, index=True)
    student_id = fields.Many2one(related='student_fee_id.student_id', string='Student', store=True, index=True)
    student_division_id = fields.Many2one(related='student_fee_id.student_division_id', string='Division',
                                          readonly=True, store=True, index=True)
    academic_year_id = fields.Many2one(related='student_fee_id.academic_year_id', string='Academic year', readonly=True,
                                       store=True, index=True)
    ...
    journal_id = fields.Many2one('account.journal', 'Receive On', copy=False,
    ...
    def reset_payment_to_draft(self):
        ...
        for invoice in invoices:
            ...
            # Bulk callers set ala_bulk_skip_line_log to avoid one mail.message
            # per line across hundreds of students; they log at run level.
            if not self.env.context.get('ala_bulk_skip_line_log'):
